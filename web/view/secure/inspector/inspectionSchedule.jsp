...
<head>
    <meta charset="UTF-8">
    <title>Inspection Schedule Requests</title>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
</head>
<body>
    <div class="container mt-4">
        <h2>Duyệt Đơn Kiểm Định Xe</h2>
        ...
        <!-- Bộ lọc -->
        <form method="get" action="${pageContext.request.contextPath}/station/inspectionSchedule" class="mb-3">
            <div class="form-row">
        ...
        <c:choose>
            <c:when test="${empty requests}">
                <p class="text-muted">Không có yêu cầu kiểm định xe nào.</p>
            </c:when>
            <c:otherwise>
                            ...
                            <tr>
                                ...
                                <td>
                                    <c:if test="${request.status == 'Pending'}">
                                        <form method="post" action="${pageContext.request.contextPath}/station/inspectionSchedule" style="display:inline;">
                                    ...
                                    <a href="${pageContext.request.contextPath}/station/inspectionSchedule?requestId=${request.requestID}&page=${currentPage}&statusFilter=${statusFilter}&searchKeyword=${searchKeyword}&fromDate=${fromDate}&toDate=${toDate}&sortBy=${sortBy}&sortOrder=${sortOrder}" title="View Details">
        ...
        <c:if test="${not empty selectedRequest && not empty user && not empty vehicle && not empty schedule}">
            <div class="mt-4">
                <h4>Chi Tiết Yêu Cầu Kiểm Định</h4>
                ...
                <h5>Thông Tin Lịch Kiểm Định</h5>
                <div class="form-group">
                    <label>Schedule Date:</label>
                    <input type="text" class="form-control" value="<fmt:formatDate value='${schedule.scheduleDate}' pattern='dd/MM/yyyy HH:mm:ss' />" readonly>
                </div>
                <div class="form-group">
                    <label>Status:</label>
                    <input type="text" class="form-control" value="${schedule.status}" readonly>
                </div>
            </div>
        </c:if>
    </div>
